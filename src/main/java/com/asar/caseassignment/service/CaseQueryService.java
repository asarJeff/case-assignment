...
public class CaseQueryService {
    ...
    public CaseQueryService(
        ...
        for (String s : openLikeStatusesCsv.split(",")) {
            String t = s.trim();
            if (!t.isBlank()) this.openLikeStatuses.add(t);
        }

        this.agentAssignmentEventCodes = new HashSet<>();
        for (String s : agentEventCodesCsv.split(",")) {
            String t = s.trim();
            if (!t.isBlank()) this.agentAssignmentEventCodes.add(t);
        }
    ...
    public List<Map<String, Object>> getAllCases() {
        ...
        System.out.println("CaseQueryService: fetching cases with NO SAP filter");
        ...
        while (true) {
            ...
            System.out.println("CaseQueryService: fetched page skip=" + skip
                    + " size=" + page.size() + " total=" + allCases.size());

            if (page.size() < batchSize) break;
            skip += batchSize;

            if (skip >= 10000) {
                System.out.println("CaseQueryService: WARNING - hit SAP 10k $skip limit. "
                        + "Total fetched so far: " + allCases.size());
                break;
            }
        }
    ...
    /**
     * Builds an OData $filter expression from the configured openLikeStatuses set.
     * Example output: "status eq 'Z2' or status eq 'Z3' or status eq '01'"
     * This filters at the SAP level so we only fetch relevant cases,
     * avoiding the 10k $skip hard limit when total case volume is high.
     */
    private String buildStatusFilter() {
        return "lifeCycleStatus ne 'COMPLETED'";
    }
    ...
    public List<Map<String, Object>> getCandidateCases() {
        ...
        for (Map<String, Object> c : all) {
            ...
            // 1) Skip claims cases — not using case assignment app
            String caseType = str(c.get("caseType"));
            if ("ZCLM".equals(caseType)) continue;

            // 2) Status filter — already applied at SAP level but kept as safety net
            String status = str(c.get("status"));
            if (!openLikeStatuses.contains(status)) continue;

            // 3) Optional extra guard on lifeCycleStatus
            String lcs = str(c.get("lifeCycleStatus"));
            if (!lcs.isBlank() && !(lcs.equals("OPEN") || lcs.equals("IN_PROCESS"))) {
                continue;
            }

            // 4) LastTrackingEvent filter
    ...
    public List<Map<String, Object>> getUnassignedOpenOrInProcessCases() {
        ...
        for (Map<String, Object> c : candidates) {
            // Unassigned = processor missing OR processor.id missing/blank
    ...
    /**
     * Reads extensions.LastTrackingEvent from a case map.
     * SAP returns custom fields under an "extensions" object, e.g.:
     *   { "extensions": { "LastTrackingEvent": "51105", ... } }
     * Returns blank string if not present so callers can treat it as "no filter applied".
     */
    @SuppressWarnings("unchecked")
    private String getLastTrackingEvent(Map<String, Object> caseMap) {
        Object ext = caseMap.get("extensions");
        if (!(ext instanceof Map<?, ?> extMap)) return "";
